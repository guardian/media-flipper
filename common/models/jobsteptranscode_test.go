...
func TestJobStepTranscodeFromMap(t *testing.T) {
	...
	if err != nil {
		...
		if result.TranscodeSettings == nil {
			...
			if result.TranscodeSettings.Name != "test" {
				t.Errorf("Got wrong transcode settings name, expected 'test' got %s", result.TranscodeSettings.Name)
			}
			if result.TranscodeSettings.Description != "test fake job settings" {
				t.Errorf("Got wrong transcode settings description: %s", result.TranscodeSettings.Description)
			}
			if result.TranscodeSettings.SettingsId != uuid.MustParse("87230B0F-8E75-474D-B8D0-5C421C9D4E56") {
				t.Errorf("Got wrong transcode settings ID, expected 87230B0F-8E75-474D-B8D0-5C421C9D4E56 got %s", result.TranscodeSettings.SettingsId)
			}
		}
		//spew.Dump(result)
	}
